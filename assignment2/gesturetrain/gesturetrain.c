...
#define BPNN_LEARNING_RATE 0.8

#define BPNN_SEED 102194
#define BPNN_MOMENTUM 0.3
#define BPNN_EPOCH 100
  ...
  if (n > 0) {
    ...
    if (!list_errors)
	 ...
	 discussed in section 3.1.2 of homework
          */
      printf("\t%.3f\t%.7f", ((double) correct / (double) n) * 100.0, err);
  } else {
    if (!list_errors)
      printf("0.0 0.0 ");
  }
  ...
  if ((net = bpnn_read(netname)) == NULL) {
	...
	make a net with:
	  imgsize inputs, 4 hiden units, and 1 output unit
          */
      net = bpnn_create(imgsize, BPNN_NUM_HIDDEN_UNITS, 1);
    } else {
...
int main(int argc, char *argv[])
{
  ...
  printf("Total training time: %.6lf seconds\n", currentTime-startTime);
